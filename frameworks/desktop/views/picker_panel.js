...
SC.PickerPanel = SC.PalettePanel.extend(SC.PickerPointer, {
  ...
  /**
    Is this pane anchored? Default is YES.
  ...
  */  
  ...
  /** @private
    ...
    shift x, y to optimized picker visibility and make sure top-left corner is
    always visible.
  */
  ...
  /** @private
    re-position rule optimized for Menu to enforce min left(7px)/right(8px) 
  ...
  */
  ...
  /** @private
    re-position rule for triangle pointer picker: take default [0,1,2,3,2] or 
    custom matrix to choose one of four perfect pointer positions.
  */
  fitPositionToScreenPointer: function(w, f, a) {
    ...
    if(!this.preferMatrix || this.preferMatrix.length != 5) {
      // default re-position rule : perfect right (0) > perfect left (1) > 
      // perfect top (2) > perfect bottom (3)
      // fallback to perfect top (2)
