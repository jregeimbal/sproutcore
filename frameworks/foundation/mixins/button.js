...
SC.Button = {
  ...
  */
  ...
  /**
    The computed display tooltip.  This is generated by using the tooltip
    specified or defaulting to the title. If tooltip is blank, will return
    NO.
  ...
  */
  ...
  }.property('title','tooltip','localize').cacheable(),
  ...
  */
  renderTitle: function(context, firstTime) {
    var icon = this.get('icon'),
        ...
        cssClasses = 'sc-button-label',
        tooltip = this.get('displayTooltip'),
    ...
    if(firstTime){
      if ( this.get('needsEllipsis') ) { cssClasses = [cssClasses,'ellipsis'].join(' '); }
      context.push('<label class="'+cssClasses+'" title="'+ (tooltip ? tooltip : "" ) +'">'+imgTitle+'</label>');
      this._ImageTitleCached = imgTitle;
    }else{
      elem = this.$('label');
      if ( (htmlNode = elem[0])) {
        if ( tooltip ) { elem.attr({title: tooltip}); }
        else { elem.attr({title: ''}); }

        if ( this.get('needsEllipsis') ) { elem.addClass('ellipsis'); }
        else { elem.removeClass('ellipsis');}
