...
SC.Module = SC.Object.create(/** @scope SC.Module */ {
  ...
  */
  loadModule: function(moduleName, target, method) {
    ...
    if (log) SC.Logger.log("SC.Module: Attempting to load '%@'", moduleName);
    ...
    if (module.isLoaded && !module.isWaitingForRunLoop) {
      if (log) SC.Logger.log("SC.Module: Module '%@' already loaded.", moduleName);
      ...
      if (module.source) {
        if (log) SC.Logger.log("SC.Module: Evaluating JavaScript for module '%@'.", moduleName);
    ...
    // the code.
      ...
      if (log) SC.Logger.log("SC.Module: Module '%@' is not loaded, loading now.", moduleName);
  ...
  */
  prefetchModule: function(moduleName) {
    ...
    if (SC.LOG_MODULE_LOADING) SC.Logger.log("SC.Module: Prefetching module '%@'.", moduleName);
    this._loadDependenciesForModule(moduleName);
    this._loadCSSForModule(moduleName);
    this._loadJavaScriptForModule(moduleName);
    module.isLoading = YES;
  ...
  */
  _executeLazilyInstantiatedModule: function(moduleName, targetName, methodName){
    ...
    if (SC.LOG_MODULE_LOADING) {
      SC.Logger.log("SC.Module: Module '%@' is marked for lazy instantiation, instantiating it now…", moduleName);
    }
  ...
  */
  _loadCSSForModule: function(moduleName) {
    ...
    for (idx = 0; idx < len; idx++) {
      ...
      if (url.length > 0) {
        if (SC.LOG_MODULE_LOADING) SC.Logger.log("SC.Module: Loading CSS file in '%@' -> '%@'", moduleName, url);
  ...
  _loadJavaScriptForModule: function(moduleName) {
    ...
    if (url.length > 0) {
      if (SC.LOG_MODULE_LOADING) SC.Logger.log("SC.Module: Loading JavaScript file in '%@' -> '%@'", moduleName, url);
  ...
  _loadDependenciesForModule: function(moduleName) {
      ...
      for (idx = 0; idx < len; idx++) {
        ...
        if (!requiredModule) {
          ...
          // Required module has not been loaded nor requested yet.
            ...
            if (log) SC.Logger.log("SC.Module: '%@' depends on '%@', loading dependency…", moduleName, requiredModuleName);
    ...
    */
    _invokeCallbacksForModule: function(moduleName) {
      ...
      if (SC.LOG_MODULE_LOADING) SC.Logger.log("SC.Module: Module '%@' has completed loading, invoking callbacks.", moduleName);
    ...
    _evaluateAndInvokeCallbacks: function(moduleName) {
      ...
      if (log) SC.Logger.log("SC.Module: Evaluating and invoking callbacks for '%@'.", moduleName);
    ...
    _moduleDidBecomeReady: function(moduleName) {
      ...
      for (var idx = 0, len = dependents.length; idx < len; idx++) {
        ...
        if (dependent.isLoaded && this._dependenciesMetForModule(dependentName)) {
          if (log) SC.Logger.log("SC.Module: Now that %@ has loaded, all dependencies for a dependent %@ are met.", moduleName, dependentName);
...
*/
  _moduleDidLoad: function(moduleName) {
    ...
    if (log) SC.Logger.log("SC.Module: Module '%@' finished loading.", moduleName);

    if (!module) {
      if (log) SC.Logger.log("SC._moduleDidLoad() called for unknown module '@'.", moduleName);
      module = SC.MODULE_INFO[moduleName] = { isLoaded: YES, isReady: YES } ;
      return;
    }

    if (module.isLoaded) {
      if (log) SC.Logger.log("SC._moduleDidLoad() called more than once for module '%@'. Skipping.", moduleName);
      return ;
    }
    ...
    if (!module.isPrefetching) {
      ...
      if (dependenciesMet) {
        ...
        if (log) SC.Logger.log("SC.Module: Dependencies for '%@' not met yet, waiting to evaluate.", moduleName);
      }
    } else {
      delete module.isPrefetching;
      if (log) SC.Logger.log("SC.Module: Module '%@' was prefetched, not evaluating until needed.", moduleName);
    }
  },
